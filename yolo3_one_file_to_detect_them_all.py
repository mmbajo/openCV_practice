    ...
    def load_weights(self, model):
        for i in range(106):
            try:
                ...
                print("loading weights of convulution #" + str(i))
...
def _conv_block(inp, convs, skip = True):
    ...
    for conv in convs:
        ...
        if conv['leaky']:
            x = LeakyReLU(alpha = 0.1, name = 'leaky_' + str(conv['layer']))(x)
...
def preprocess_input(image, net_h, net_w):
    ...
    if (float(net_w)/new_w) < (float(net_h)/new_h): # ratio comparison
    ...
    # resize the image tothe new size
